...
if(isServer) then {
	...
	[_mission,_position,"medium","Renegade Base","MainBandit",true] call mission_init;
	
	diag_log 		format["WAI: [Mission: Renegade Base]: Starting... %1",_position];
	...
	_complete = [
		...
		"Renegades have setup a heavily fortified base",	// mission announcement
		"Survivers have captured the bandit base",		// mission success
		"Survivers did not capture the base in time"	// mission fail
	] call mission_winorfail;

	if(_complete) then {
		[_crate,[18,ai_wep_box],[8,crate_items],[3,crate_items_high_value],[4,crate_backpacks_large]] call dynamic_crate;
	};
