...
if(isServer) then {
	...
	[_mission,_position,"easy","Mi-8 Crash","MainBandit",true] call mission_init;

	diag_log 		format["WAI: [Mission: Mi-8 Crash]: Starting... %1",_position];

	_crate = [0,_position] call wai_spawn_create;
	
	_baserunover 	= createVehicle ["Land_Wreck_Heli_Attack_02_F",[((_position select 0) + 15), ((_position select 1) + 15), 0],[],10,"FORM"];
	...
	_complete = [
		...
		"An Mi-8 carrying supplies has crashed and renegades are securing the site! Check your map for the location!",	// mission announcement
		"Survivors have secured the crashed Mi-8.",											// mission success
		"Survivors did not secure the crashed Mi-8 in time"							// mission fail
	] call mission_winorfail;
	
	if(_complete) then {
		[_crate,[18,ai_wep_box],[6,crate_items],[7,crate_items_high_value],[4,crate_backpacks_large]] call dynamic_crate;
	};

	diag_log format["WAI: [Mission: Mi-48 Crash]: Ended at %1",_position];
