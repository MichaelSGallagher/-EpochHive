if(isServer) then {
    ...
    [_mission,_position,"easy","MV-22 Crash","MainBandit",false] call mission_init;
    diag_log         format["WAI: [Mission: MV_22]: Starting... %1",_position];
...
//    _wreck             = createVehicle ["Submarine_01_F",[(_position select 0), (_position select 1), 0],[],10,"CAN_COLLIDE"];
    ...
    //Spawn sharks    
    0 = _position spawn {
        waitUntil( { isPlayer _x && _x distance _position < 500 } count allPlayers > 0 );
    ...
    _complete = [
        ...
        "An MV-22 Osprey carrying supplies has crashed into the sea. Salvage what you can.",    // mission announcement
        "Survivors have secured the crashed Osprey.",    // mission success
        "Survivors did not secure the crashed Osprey in time."        // mission fail
    ] call mission_winorfail;

    diag_log format["WAI: [Mission:[MV_22]: Ended at %1",_position];
